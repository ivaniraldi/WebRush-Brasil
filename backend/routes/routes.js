...
const transporter = nodemailer.createTransport({
  ...
  secure: true, // Usa SSL
  auth: {
    user: process.env.EMAIL_USER, // Ejemplo: contato@webrushbrasil.com.br
...
router.post("/contacts", async (req, res) => {
  ...
  if (!name || !email || !message) {
    return res
      .status(400)
      .json({ error: "Faltan datos requeridos: name, email o message" });
  }
...
router.post("/blog", async (req, res) => {
  ...
  if (!title || !slug || !content) {
    return res
      .status(400)
      .json({ error: "Faltan datos requeridos: title, slug o content" });
  }
